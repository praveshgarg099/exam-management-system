...
public class testtake extends JFrame {
	...
	private void initExamSession() {
		try {
			...
			// 1. Resolve Student
			if (currentSession != null && currentSession.getStudentId() != null) {
				Student s = studentService.getStudentById(currentSession.getStudentId());
				if (s != null) {
			...
			} else if (currentSession != null && currentSession.getStudentId() != null) {
				List<ExamSchedule> list = examService.getSchedulesByStudentId(currentSession.getStudentId());
			...
			if (activeSchedule != null) {
				Subject sub = subjectService.getSubjectById(activeSchedule.getSubjectId());
				if (sub != null) {
				...
				// Load randomized stable questions for this schedule
				sessionQuestions = examService.prepareExamQuestions(
						activeSchedule.getId(),
						activeSchedule.getSubjectId(),
						activeSchedule.getTotalQuestions()
				);

				int mins = activeSchedule.getDurationMinutes();
				second = 0;
				minute = Math.max(1, mins);
			} else {
				// Fallback if launched standalone without schedule
				Subject defaultSub = subjectService.getAllSubjects().stream().findFirst().orElse(null);
				if (defaultSub != null) {
					lblNewLabel_3.setText(defaultSub.getName());
					QuestionService qs = new QuestionService();
					sessionQuestions = qs.getActiveQuestionsBySubjectId(defaultSub.getId());
		...
		try {
			if (sessionQuestions != null) {
				for (int i = 1; i <= sessionQuestions.size(); i++) {
					...
					Question q = sessionQuestions.get(i - 1);
					String correctAns = q.getCorrectAnswer();
					if (userAns != null && correctAns != null && !userAns.trim().isEmpty() && userAns.trim().equalsIgnoreCase(correctAns.trim())) {
						correctCount++;
					}
				}
			}
			...
			ExamResult result = resultService.submitExamResult(scheduleId, studentId, subjectId, totalQuestions, correctCount, passThreshold);
